...
LogLikeBMAPFn <- function(beta0,TT,numParms,prior){
  ...
  # delta.x  = prior$gamma%*%matrix(t(beta[,-2]),ncol = 1)-prior$mu
...
LogLikeBMAPGr <- function(beta0,TT,numParms,prior){
  ...
  delta.x=prior$gamma%*%beta0[-length(beta0)]-prior$mu
  # delta.x = prior$gamma%*%matrix(t(beta[,-2]),ncol = 1)-prior$mu
  temp = t(t(prior$gamma)%*%solve(prior$sigma,delta.x))
  # temp = matrix(temp,numParms-1)
  # temp = c(rbind(temp[1,],0,temp[2,]))
  gg=cbind(matrix(t(gg[,1:numParms]),1)+temp,sum(gg[,numParms+1]))
...
CalHessian<-function(x,sigma,TT,numParms,modelCountry,randEff){
  ...
  if(randEff==1){
    obj = sparseHessianFD(x,fn=LogLikeBetaAllFn,gr=LogLikeBetaAllGr
                          ,rows=mc$rows
                          ,cols=mc$cols
                          ,sigma=sigma
                          ,TT=TT
    ...
    AA=rbind( LogLikeBetaAllGr(sigma=c(sigma1Plus ,sigma[2]   ),x=x,TT=TT,numParms=numParms,modelCountry=modelCountry)
             -LogLikeBetaAllGr(sigma=c(sigma1Minus,sigma[2]   ),x=x,TT=TT,numParms=numParms,modelCountry=modelCountry),
              LogLikeBetaAllGr(sigma=c(sigma[1]   ,sigma2Plus ),x=x,TT=TT,numParms=numParms,modelCountry=modelCountry)
             -LogLikeBetaAllGr(sigma=c(sigma[1]   ,sigma2Minus),x=x,TT=TT,numParms=numParms,modelCountry=modelCountry))/epsilon
    
    B11   =   LogLikeBetaAllFn(sigma=c(sigma1Plus ,sigma[2]   ),x=x,TT=TT,numParms=numParms,modelCountry=modelCountry)-
            2*LogLikeBetaAllFn(sigma=c(sigma[1]   ,sigma[2]   ),x=x,TT=TT,numParms=numParms,modelCountry=modelCountry)+
              LogLikeBetaAllFn(sigma=c(sigma1Minus,sigma[2]   ),x=x,TT=TT,numParms=numParms,modelCountry=modelCountry)
    
    B12=B21 = LogLikeBetaAllFn(sigma=c(sigma1Plus ,sigma2Plus ),x=x,TT=TT,numParms=numParms,modelCountry=modelCountry)-
              LogLikeBetaAllFn(sigma=c(sigma1Plus ,sigma2Minus),x=x,TT=TT,numParms=numParms,modelCountry=modelCountry)-
              LogLikeBetaAllFn(sigma=c(sigma1Minus,sigma2Plus ),x=x,TT=TT,numParms=numParms,modelCountry=modelCountry)+
              LogLikeBetaAllFn(sigma=c(sigma1Minus,sigma2Minus),x=x,TT=TT,numParms=numParms,modelCountry=modelCountry)
    
    B22     = LogLikeBetaAllFn(sigma=c(sigma[1]   ,sigma2Plus ),x=x,TT=TT,numParms=numParms,modelCountry=modelCountry)-
            2*LogLikeBetaAllFn(sigma=c(sigma[1]   ,sigma[2]   ),x=x,TT=TT,numParms=numParms,modelCountry=modelCountry)+
              LogLikeBetaAllFn(sigma=c(sigma[1]   ,sigma2Minus),x=x,TT=TT,numParms=numParms,modelCountry=modelCountry)
    
    BB=rbind(c(B11,B12),c(B21,B22))/(.5*epsilon)^2
    hess=rbind(cbind(hess,t(AA)),cbind(AA,BB))
  }
  if(randEff==0){
    obj = sparseHessianFD(c(x,sigma),fn=LogLikeBMAPAllFn,gr=LogLikeBMAPAllGr
                          ,rows=mc$rows
                          ,cols=mc$cols
                          ,TT=TT
                          ,numParms=numParms
