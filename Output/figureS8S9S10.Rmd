...
```{r echo=FALSE, fig.asp=0.8, fig.show='hold', fig.width=12, message=FALSE, warning=FALSE, dpi=300}
rm(list = ls())
library(haven)
library(ggrepel)
library(expint)
...
p1Data = map_dfr(c(files), function(ff) {
  map_dfr(c("Male", "Female"), function(ss) {
    load(paste0('Estimates/priorA/c', cc, ss, 'B0B1var', ff, '.rData'))
    numParms = 2
    x = result$x
    sigma = result$sigma
...
p3Data = map_dfr(c(files), function(ff) {
  map_dfr(c("Male", "Female"), function(ss) {
    ...
    numParms = 2
    modelCountry = result$modelCountry
    load(paste0('MH/MHc', cc, ss, 'var', ff, '.rData'))
    z = result$zSample
    Total = sum(!is.na(z[1,]))
...
merged_all_double = map_dfr(c(files), function(ff) {
  map_dfr(c("Male", "Female"), function(ss) {
    read_rds(paste0("Estimates/CI/", cc, ss, "CI", ff, ".rds")) %>%
...
```
```{r echo=FALSE, fig.asp=0.4, fig.show='hold', fig.width=12, message=FALSE, warning=FALSE, dpi=300}
temp %>%
  dplyr::summarise(
